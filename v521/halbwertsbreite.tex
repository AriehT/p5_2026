...
Der für einen Halbleiterdetektor zu erwartende theoretische Zusammenhang (bei dem die intrinsische Halbwertsbreite $\Delta E_d$ mit der Photonenenergie zunimmt) wird durch folgende Gleichung beschrieben:
\begin{equation}
\Delta E (E_\gamma) = \sqrt{\Delta E_d(E_\gamma)^2+\Delta E_e^2}.
\label{eq_2}
\end{equation}

Der intrinsische Anteil der Halbwertsbreite ergibt den Zusammenhang welches in Gleichung dargestellt ist:
\begin{equation}
...
Abbildung \ref{fig:halbw_1_2} zeigt die in Abhängigkeit von der Energie berechneten FWHM-Werte für beide Detektoren sowie die entsprechenden, aus den Gleichungen \ref{eq_2} und \ref{eq_3} abgeleiteten Funktionen.
\begin{figure}[htbp]
    \centering
...
Dies ist eine direkte Folge der unterschiedlichen Detektionsmechanismen: beim HPGe-Detektor entstehen pro MeV etwa $3\times10^5$ Elektron-Loch-Paare, deren statistische Schwankungen gering sind. Beim NaI(Tl)-Detektor wird die Energie zunächst in Szintillationslicht umgewandelt, dessen Photonenzahl starken statistischen Schwankungen unterliegt. Dazu führen Lichtsammlungsverluste und Verstärkungsvariationen im Photomultiplier zu einer weiteren Verbreiterung der Peaks.
...
Im Gegensatz dazu beträgt der reduzierte $\chi^2$-Wert des HPGe-Detektors lediglich $2,23$.
Die Abweichung von $\chi^2_{\mathrm{red}}=1$ ist auf die leicht asymmetrische Peakform zurückzuführen. Beim NaI(Tl)-Detektor sind die Abweichungen größer, da die Peakform durch Compton-Anteile und Lichtsammlungsverluste nicht ideal gaußförmig ist.
Die Bestimmung der FWHM bleibt dennoch stabil, da die Peakflanken gut erfasst werden.
 Obwohl dieser Wert nicht exakt Eins entspricht, beschreibt das Modell die Messdaten insgesamt gut. Die zugrunde liegenden Messwerte sind in Tabelle \ref{tab:fwhm_messwerte} aufgeführt.
