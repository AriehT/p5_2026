...
\begin{align*}
    \varepsilon(E_\gamma) \approx \frac{N_{\text{detektiert}}}{B \cdot P_\gamma \cdot t}\cdot \frac{4\pi D^2}{\pi d^2}.
\end{align*}
...
Die Schwellenwertfunktion \(B(\text{Cs})\) sowie der Abstand sind Tabelle \ref{tab_just1} zu entnehmen. Basierend auf der Detektionsrate \(N_\text{detect} = A\) (Tabellen \ref{taba_2} und \ref{taba_3}), die so bestimmten Effizienzen betragen für die $661keV$-Linie:
\begin{align*}
...
Analog zum Cäsium-Fall lassen sich die Effizienzen für das gesamte Europium-Spektrum bestimmen. Abbildung \ref{fig:eff_1} stellt die erzielten Ergebnisse in Form einer Effizienzkurve dar.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/Ge_effizienz_richtig.png}
...
$a_1 = (0.202733 \pm 0.017735), \qquad a_2 = (-0,053473 \pm 0,002698)$\\
...
Die Effizienz des HPGe-Detektors ist bei niedrigen Energien deutlich höher, da der Photoeffekt dominiert und die Photonen vollständig im Kristall absorbiert werden. Aufgrund der dominierenden Compton-Streuung bei hohen Energien und der begrenzten Kristallgröße verringert sich die Detektionseffizienz, da die Photonenenergie nicht mehr vollständig deponiert werden kann.

Trotz dieser Einschränkung liefert die Regression für den Szintillationsdetektor eine konsistente Beschreibung des Effizienzverhaltens im Energiebereich oberhalb von etwa $200keV$ und stimmt mit der aus der Cäsium-Messung bestimmten Effizienz bei $661keV$ überein. Sie liefert somit eine konsistente Darstellung der Detektoreffizienz in Abhängigkeit von der Energie im untersuchten Bereich.

Die Effizienzkurve bildet die Grundlage für die Aktivitätsbestimmung der $^{60}$Co-Quelle im nächsten Kapitel, da die gemessenen Peakflächen dort mit der energieabhängigen Effizienz gewichtet werden müssen.
