...
Die Wahrscheinlichkeit, dass ein $\gamma$-Photon mit der Energie $E$ im Detektor ein vollständiges Signal erzeugt, wird mit der Detektoreffizienz $\epsilon(E)$ beschrieben. Diese hängt von der Anzahl der detektierten $\gamma$-Photonen $N_\gamma(E)_{detektiert}$ und der $N_{erreicht}$ ab:
\begin{equation}
...
Die Schwellenwertfunktion \(B(\text{Cs})\) sowie der Abstand sind Tabelle \ref{} zu entnehmen. Basierend auf der Detektionsrate \(N_\text{detect} = A\) (Tabellen \ref{} und \ref{}), die dem 661-keV-Photopeak zugeordnet ist, ergeben sich folgende optimale Werte:
...
Analog zum Cäsium-Fall lassen sich die Effizienzen für das gesamte Europium-Spektrum bestimmen. Dabei ist zu beachten, dass die Referenzfunktion \(B(E)\) für jede Energie \(E\) mit der entsprechenden Zählrate verglichen werden muss (siehe Tabelle \ref{}). Abbildung 11 stellt die erzielten Ergebnisse in Form einer Effizienzkurve dar.
...
Abbildung \ref{fig:eff_1} zeigt, dass die Effizienz \(\epsilon(E)\) beider Detektoren mit zunehmender Energie im Bereich \(E \in [90\,\text{keV}, 1400\,\text{keV}]\) deutlich abnimmt. Dieses Verhalten ist bekannt und lässt sich durch die folgende empirische Anpassungsfunktion beschreiben \ref{}:
